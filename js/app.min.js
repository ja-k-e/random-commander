(function() {
    ...
    app.controller("EditorCtrl", [ "$scope", function($scope) {
        ...
        return isOpera = !!window.opera || navigator.userAgent.indexOf(" OPR/") >= 0, isFirefox = "undefined" != typeof InstallTrigger, 
        isSafari = Object.prototype.toString.call(window.HTMLElement).indexOf("Constructor") > 0, 
        isChrome = !!window.chrome && !isOpera, isIE = !1 || !!document.documentMode, $scope.browser = {
            opera: isOpera,
            firefox: isFirefox,
            safari: isSafari,
            chrome: isChrome,
            ie: isIE
        }, $scope.library = DataLibrary, $scope.about = !1, $scope.toggleAbout = function() {
            return $scope.menu = !1, $scope.about = !$scope.about;
        }, $scope.metronome = !1, $scope.toggleMetronome = function() {
            return $scope.metronome = !$scope.metronome;
        }, $scope.menu = !1, $scope.toggleMenu = function() {
            return $scope.about = !1, $scope.menu = !$scope.menu;
        ...
        $scope.progress = 0, $scope.playPerformance = function() {
            ...
            beats_per_measure = composition.beats, resolution = composition.resolution, performance = $scope.performance, 
            beats = composition.measures * beats_per_measure * resolution / 4, index = 0, tempo_time = 6e4 / tempo, 
            next_beat = function() {
                ...
                for (i = _i = 0, _len = performance.length; _len > _i; i = ++_i) {
                    if (sequence = performance[i], chord = sequence[index], $scope.metronome && index % (resolution / 4) === 0 && (freq = 0 === index || index / 4 % beats_per_measure === 0 ? 4100 : 3e3, 
                    ...
                    decibels.push(composition.clefs.bass.decibels)), sustain = chord.sustain, notes_size = chord.notes.length, 
                    ...
                    selector = "." + clef + " .beat:nth-child(" + (index + 1) + ")", chord && "sus" !== chord && ($("." + clef + " .beat.active").removeClass("active"), 
                    $(selector).addClass("active"));
                }
                return index = (index + 1) % beats;
            }, next_beat(), time = tempo_time / (resolution / 4), performance_interval = window.setInterval(next_beat, time);
            ...
            getPerformance: function(composition) {
                ...
                for (index in composition.clefs) {
                    ...
                    random > clef.silence) {
                        ...
                        note_length = 1 / value), bps = composition.tempo / 60, beat_count = note_length / (1 / composition.resolution), 
                        res_beats = 4 / composition.resolution, beat_sus = res_beats / bps, sustain = beat_count * beat_sus, 
                        sustain_classname = Math.round(1e3 * sustain), note_whole = Math.floor(note_length / step_length), 
                        note_decimal = Math.floor(100 * (note_length / step_length - note_whole)), note_width = note_whole + "_" + note_decimal, 
                        new_chord = {
                            length: note_length,
